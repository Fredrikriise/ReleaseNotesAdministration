...
namespace test.ReleaseNotesTests.Controllers
{
    public class ReleaseNoteControllerTest
    {
        ...
        public async Task ListAllReleaseNotes_Should_Return_View()
        {
            ...
            var converted = JsonConvert.DeserializeObject<List<ReleaseNoteApiModel>>(content);

            // 3. "map" this to ProductViewModel objects
            var mappedJson = converted.Select(x => new ReleaseNoteViewModel
            {
                Title = x.Title,
                BodyText = x.BodyText,
                Id = x.Id,
        ...
        public async Task ListReleaseNotesForProduct_Should_Return_View_With_List()
        {
            ...
            // 3. "map" this to ProductViewModel objects
            var releaseNotesList = converted.Where(x => x.ProductId == productId).Select(x => new ReleaseNoteViewModel
            {
                Title = x.Title,
                BodyText = x.BodyText,
                Id = x.Id,
